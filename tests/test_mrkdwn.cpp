...
TEST_CASE("user mention with label", "[mrkdwn]") {
    checkOne(MrkdwnParser::parse("<@U123|alice>"), "alice", EntityType::UserMention, 0, 5, "U123");
}

TEST_CASE("channel mention", "[mrkdwn]") {
    checkOne(
        MrkdwnParser::parse("<#C456|general>"), "#general", EntityType::ChannelMention, 0, 8, "C456"
    );
}

TEST_CASE("link no label", "[mrkdwn]") {
    checkOne(
        MrkdwnParser::parse("<https://example.com>"),
        "https://example.com",
        EntityType::Link,
        0,
        19,
...
TEST_CASE("link with label", "[mrkdwn]") {
    checkOne(
        MrkdwnParser::parse("<https://example.com|click here>"),
        "click here",
        EntityType::Link,
        0,
        10,
