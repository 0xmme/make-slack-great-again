...
When you're done, you'll have one value: the app's client ID. If you use the prebuilt app, go to **Settings → System**, find **Microsoft Teams**, paste it there and click **Save** — no rebuild needed. If you build your own msga, you can instead drop it into `credentials.cmake` (see [Step 3](#step-3-add-credentials-to-the-build)) and follow the build steps in the [README](../README.md).
...
**Prebuilt app:** open **Settings → System**, scroll to **Microsoft Teams**, paste the client ID from Step 1 and click **Save**. It applies the next time you add a Teams workspace. A client ID saved here also overrides any value compiled into the build.

**Building yourself:** copy `credentials.cmake.example` to `credentials.cmake` in the project root (if you haven't already) and fill in the Teams client id from Step 1:
